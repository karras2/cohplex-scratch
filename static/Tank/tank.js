function Tank(radius, rotate) {
  ...
  this.setCanvasSize = function(camera) {
    ...
    this.canvasPos = {
      x: (this.radius * this.bodySize * camera.z + xx),
      y: (this.radius * this.bodySize * camera.z + yy)
    };
    for (let i = 0; i < this.guns.length; i++) {
  ...
  this.draw = function(ctx, camera) {
    if (this.opacity >= 1) {
      this.canvasPos = {
        x: (this.x - this.dx - camera.x) * camera.z,
        y: (this.y - this.dy - camera.y) * camera.z
      };
      ctx.save();
      ctx.globalAlpha = 1;
...
function Auto5() {
  ...
  for (let dir = -Math.PI / 5 * 4; dir <= Math.PI; dir += Math.PI / 5 * 2) {
    this.guns.push(new AutoGun([0, 0.8], 0, 0.5, [
      [0, 0],
      [0.3, 0],
      [0.3, 1.08],
      [-0.3, 1.08],
