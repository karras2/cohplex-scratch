...
exports.baseKill = function(t) {
  t.health -= util.isF(t.maxHealth) / 60;
}
...
exports.setUserLevel = function(user) {
  ...
  let expList = [
    ...
    21849,
    23536
  ]; // The xps the players get points on.
...
exports.upgradeTank = function(user, socket) {
  ...
  let gun = {
    ...
    event: {
      killEvent: function(u, deader) {
        if (!u.owner) return;
        u.owner.controlObject.exp += deader.exp;
        return true;
      },
...
exports.setUserTank = function(user, BOT) { // 유저 총구 설정.
  ...
  user.speed = function() {
    return (0.07 + (0.007 * user.stats[7])) * Math.pow(0.985, user.level - 1);
  };
  user.damage = function() {
    return 20 + user.stats[2] * 4;
  };
  user.radius = function() {
    return (13 * Math.pow(1.01, (user.level - 1))) * (user.realSize || 1);
  };
  user.maxHealth = function() {
    return 48 + user.level * 2 + user.stats[1] * 20;
  };
  user.bound = 1;
  user.stance = 1;
  user.guns = [];
  ...
  user.isOwnCol = false;
  user.isCanDir = true;
  user.tankType = names[user.type];
  if (!BOT) user.event = { // 여기 있는 값들은 모두 "함수" 입니다.
  ...
  if ([43, 44, 45, 27, 16].includes(user.type)) user.realSize = 3;
  else if ([60].includes(user.type)) user.realSize = 2;
  else user.realSize = 1;
  ...
  switch (user.type) {
    ...
    case 27: // MotherShip
      ...
      user.maxHealth = function() {
        return 7000 + user.level * 2;
      };
      user.speed = function() {
        return 0.01 * Math.pow(0.985, user.level - 1);
      };
      user.stance = 0;
      break;
    case 28: // Predator
    ...
    case 57: // Commander
      ...
      break;
    case 58: // Mega Smasher
      user.maxStats = [10, 10, 10, 0, 0, 0, 0, 10];
      user.maxHealth = function() {
        return 75 + user.level * 2 + user.stats[1] * 20;
      };
      break;
    case 59: { // Hoinfodaman
      let list = [];
        ...
        let gun = getDroneGun({
          ...
          event: {
            ...
            deadEvent: function(u, killer) {
              for (let i = 0; i < u.guns.length; i++) {
                u.guns[i].autoShot = true;
                u.guns[i].bulletLimit++;
              }
              return true;
            }
          }
        });
        gun.event.deadEvent = function(u, killer) {
          for (let i = 0; i < u.guns.length; i++) u.guns[i].autoShot = true;
          gun.bulletLimit++;
          return true;
        };
        user.guns.push(gun);
  ...
  case 60: { // Fallen Overlord
    ...
    user.maxHealth = function() {
      return 4e3 + user.level * 2 + user.stats[1] * 20;
    };
    user.speed = function() {
      return 0.025 * Math.pow(0.985, user.level - 1);
    };
    user.stance = 0.01;
  }
  break;
